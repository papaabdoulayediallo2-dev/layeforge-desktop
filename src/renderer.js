...
const state = {
  ...
  config: {
    ...
    // ── Sécurité & Protection ──────────────────────────────
    security: {
      installPassword: { enabled: false, value: '' },
      launchPassword:  { enabled: false, value: '' },
      hwid:            { enabled: false },
...
document.addEventListener('DOMContentLoaded', () => {
  initWindowControls();
  initDragDrop();
  // Écouteur pour les logs de build
...
function initStep2() {
  ...
  $('btn-next-2').addEventListener('click', () => {
    ...
    state.config.security = {
      installPassword: {
        enabled: $('sec-install-pwd-enabled').checked,
        value:   $('sec-install-pwd').value.trim()
      },
      launchPassword: {
        enabled: $('sec-launch-pwd-enabled').checked,
        value:   $('sec-launch-pwd').value.trim()
      },
      hwid: {
        enabled: $('sec-hwid-enabled').checked
      },
      trial: {
        enabled: $('sec-trial-enabled').checked,
        days:    parseInt($('sec-trial-days').value) || 30
      }
    };
  ...
  $('sec-install-pwd-enabled').addEventListener('change', (e) => {
    $('sec-install-pwd-field').style.display = e.target.checked ? 'block' : 'none';
  });
  // Code d'accès au lancement
  $('sec-launch-pwd-enabled').addEventListener('change', (e) => {
    $('sec-launch-pwd-field').style.display = e.target.checked ? 'block' : 'none';
  });
  // Verrouillage HWID
  $('sec-hwid-enabled').addEventListener('change', (e) => {
    $('sec-hwid-info').style.display = e.target.checked ? 'block' : 'none';
  });
  // Période d'essai
  $('sec-trial-enabled').addEventListener('change', (e) => {
    $('sec-trial-field').style.display = e.target.checked ? 'block' : 'none';
  });
...
async function generateProject() {
  ...
  const result = await window.electronAPI.generateProject({
    ...
    security: state.config.security   // ← Options de protection
  });
